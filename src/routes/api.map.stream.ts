...
export const Route = createFileRoute('/api/map/stream')({
  server: {
    handlers: {
      GET: async () => {
        const retryMs =
        ...
        const { subscribeOccupancyMap } = await import('../server/mapHub')
        ...
        const stream = new ReadableStream<Uint8Array>({
          start(controller) {
            ...
            unsubscribe = subscribeOccupancyMap((map) => {
              try {
                controller.enqueue(
                  encoder.encode(
                    map ? sseEvent('map', map) : sseEvent('clear'),
                  ),
                )
              } catch {
