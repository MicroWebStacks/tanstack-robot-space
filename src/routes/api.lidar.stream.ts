...
export const Route = createFileRoute('/api/lidar/stream')({
  server: {
    handlers: {
      GET: async () => {
        const retryMs =
        ...
        const { subscribeLidarScan } = await import('../server/lidarHub')
        ...
        const stream = new ReadableStream<Uint8Array>({
          start(controller) {
            ...
            unsubscribe = subscribeLidarScan((scan) => {
              try {
                controller.enqueue(
                  encoder.encode(
                    scan ? sseEvent('scan', scan) : sseEvent('clear'),
                  ),
                )
              } catch {
